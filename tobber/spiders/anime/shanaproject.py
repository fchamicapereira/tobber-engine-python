...
class Shanaproject(Indexer):
    ...
    def parse(self, response):
        ...
        for i in range(1,len(row)):
            ...
            title       = row[i].xpath('./div[@class="rel_x"]//div[@class="release_text_contents"]/text()')
            category    = row[i].xpath('./div[@class="release_row_first"]//div[@class="release_subber"]//div//a/text()')
            torrent     = row[i].xpath('./div[@class="release_row_first"]//a[@type="application/x-bittorrent"]/@href')
            size        = row[i].xpath('./div[@class="release_row_first"]//div[contains(@class,"release_size")]/text()')
            ...
            yield Torrent(
                ...
                category    = self.extract_data(category) if len(category) > 0 else None,
